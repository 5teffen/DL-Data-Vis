...
vis = [[-4.97215608, -0.01481271, -2.16974428],
[-0.80071556, 4.72880712, 3.36074645],
[-1.44505698, -1.76451157, 7.01200288]]


# --- Choose target centres --- 
cen = vis
...
voronoi_list = pyvoro.compute_voronoi(cen,[range_x, range_y, range_z],1.0)

cluster = voronoi_list


def point_line(p1,p2,label,density=4):
	delta_x = p2[0]-p1[0]
	delta_y = p2[1]-p1[1]
	delta_z = p2[2]-p1[2]
...
for cluster in voronoi_list:
	...
	for f in faces:
		...
		all_labels += (len(one_face)*[cluster_no])
...
ax.set_xlim3d(range_x)
ax.set_ylim3d(range_y)
ax.set_zlim3d(range_z)
plt.axis('off')
# ax.set_aspect('equal')
...
for i in range(len(all_faces)):
	ax.scatter3D(all_faces[i][0], all_faces[i][1], all_faces[i][2], c=colors[all_labels[i]], s=10)

for c in range(len(cen)):
	ax.scatter3D(cen[c][0], cen[c][1], cen[c][2], c=colors[c], s=60, marker = 'h')
